...
Daily aggregator of Delaware-located taildragger classified listings,
...
repos' approach, with the state filter swapped to "DE".
...
- Each fetched page's listing table is parsed back into `(title, url,
  ...
  Location cell ends in `, DE` (case-insensitive), then de-duplicated by
...
- The result is rendered into `docs/index.html` titled **"Taildragger Ads
  in Delaware"**, using the exact same table/card layout, dark-mode
...
4. GitHub will publish the page at `https://taildraggers.github.io/delaware/`
...
```html
<iframe
  src="https://taildraggers.github.io/delaware/"
  title="Taildragger Ads in Delaware"
...
- This repo could be copied/adapted for other states by changing
  `STATE_ABBR` and `_DELAWARE_LOCATION_RE` in `main.py`, and updating
  `PAGE_TITLE`.
