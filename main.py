"""Aggregates Delaware-located listings from every taildraggers.com
...
ones whose location ends in ", DE".
...
PAGE_TITLE = "Taildragger Ads in Delaware"
STATE_ABBR = "DE"
...
_DELAWARE_LOCATION_RE = re.compile(r",\s*DE\s*$", re.IGNORECASE)


def _is_delaware(location: str) -> bool:
    return bool(_DELAWARE_LOCATION_RE.search(location.strip()))
...
def collect_listings() -> list[Listing]:
    ...
    for repo in SOURCE_SITES:
        ...
        delaware_rows = [row for row in rows if _is_delaware(row.location)]
        print(f"  [{repo}] {len(rows)} listings, {len(delaware_rows)} in Delaware")
        all_listings.extend(delaware_rows)
...
def render_html(listings: list[Listing]) -> str:
    ...
    rows_html = "".join(rows) if rows else (
        ...
        "No Delaware listings found in the latest run.</td></tr>"
    )
...
def main() -> None:
    ...
    print(f"[main] total unique Delaware listings: {len(listings)}")
